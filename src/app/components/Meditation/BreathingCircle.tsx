          ...
          {modes.map((m) => {
            ...
            return (
              <button
                ...
                className={`whitespace-nowrap text-[9px] md:text-[12px] py-2 md:py-2.5 px-3 md:px-8 rounded-full border transition-all flex-shrink-0 ${
                  isActive ? 'bg-[#C7D2FE] border-transparent text-white' : 'bg-white border-neutral-200 text-neutral-400 hover:bg-neutral-50'
                } ${isSettingsLocked ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}
              >
                {m.label}
              </button>
            );
